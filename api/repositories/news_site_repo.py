...
class NewsSiteRepository:
    ...
    def find_articles(
        ...
        *,
        ...
        rows = self.conn.execute(
            f"""
            SELECT id, site_id, url, title, published, created_at
    ...
    def record_sent_articles(
        self, site_id: int, articles: list[dict[str, object]]
    ) -> None:
        self.conn.executemany(
            """
            INSERT OR IGNORE INTO news_site_articles (site_id, url, title, published)
            VALUES (?, ?, ?, ?)
            """,
            [
                (
