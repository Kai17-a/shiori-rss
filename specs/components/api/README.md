...
- GitHub releases: `GET/POST /github-repositories` で登録一覧・追加、`POST /github-repositories/refresh` で全登録先の最新公開リリースを更新し、`DELETE /github-repositories/{id}` で削除する。登録時にも最新1件を取得して保存する。公開リポジトリは認証なしで利用でき、`GITHUB_TOKEN` が設定されていればGitHub APIリクエストに使用する。
- GitHub release notifications: 定期バッチは登録リポジトリの最新公開リリースを確認し、最後に通知成功したタグから変化した場合だけ、既存の有効なWebhook全件へ通知する。1件以上成功した場合に通知済みタグを更新し、全送信が失敗した場合は次回再試行する。画面からの一括更新は表示キャッシュだけを更新し、未通知判定を失わない。
