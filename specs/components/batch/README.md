...
- AI解析は開始、記事単位の処理・使用トークン、失敗、最終件数を標準エラーへ出力し、手動実行時はAPIログへリアルタイム転送される。実行中は対象件数、処理済み件数、成功・失敗・解析済みスキップ件数、現在の記事タイトル、当日トークン使用量、開始時刻を `app_settings.ai_article_analysis_progress` のJSONへ記事単位で更新し、実行ロック解放時に削除する。
