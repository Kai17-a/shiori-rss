...
2. SQLite FTS5の三文字トークナイザーで、両記事テーブルから同期された検索インデックスを検索する。3文字未満の検索語だけの場合は部分一致検索を使う。
3. 上位候補の記事タイトル、保存済みサマリー、出典情報をLLMへ渡す。
4. LLMは候補の関連性を判断し、参照番号付きで回答する。
5. APIは回答と候補記事のURLを返し、画面は出典リンクを表示する。
