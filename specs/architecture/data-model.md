...
erDiagram
    ...
    DOCKER_IMAGES ||--o{ DOCKER_IMAGE_WEBHOOKS : selects
    WEBHOOK_ENDPOINTS ||--o{ DOCKER_IMAGE_WEBHOOKS : receives
    ...
    DOCKER_IMAGES {
        ...
        TEXT display_name
        TEXT latest_digest
        TEXT latest_notified_digest
        TEXT fetched_at
        TEXT created_at
        TEXT updated_at
    }
    DOCKER_IMAGE_WEBHOOKS {
...
`docker_images` は公開レジストリのホスト、repository、tagと現在のmanifest digestを保持する。`(registry, repository, tag)` は一意で、`latest_notified_digest` はWebhook送信が1件以上成功した最後のdigestを保持する。`docker_image_webhooks` の通知先が未選択なら通知せず、画面からの更新は `latest_digest` だけを更新する。
