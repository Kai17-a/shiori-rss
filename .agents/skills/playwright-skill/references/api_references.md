...
This document contains advanced Playwright API patterns. For CLI-first execution patterns, see [SKILL.md](SKILL.md).
...
Before using this skill, ensure Bun is available:

```bash
bun --version

# Check the CLI wrapper prerequisites
command -v bun >/dev/null 2>&1
```

### Basic Configuration

Create `playwright.config.ts` only if you are writing Playwright test files:
...
export default defineConfig({
  ...
  reporter: 'html',
  use: {
    baseURL: process.env.PLAYWRIGHT_FRONTEND_BASE_URL ?? 'http://127.0.0.1:3000',
...
The official skill prefers `playwright-cli` plus snapshot-driven element refs for terminal automation. Use the Playwright test API only when the user explicitly wants tests.
...
Set headers via environment variables when you need the browser to identify automated traffic:
...
These headers are automatically applied when you create a browser context that merges `extraHTTPHeaders`.
...
# Slow motion
bun run e2e -- --headed --slowmo=1000
```
...
2. **Selector Strategy** - Prefer data-testid attributes and role-based selectors
